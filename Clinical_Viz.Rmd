...
combined_plot <- viz_1|viz_2

ggsave(
  filename = "sex_treatment_plots_combined.png",
  plot = combined_plot,
  width = 16,   # roughly double the width since plots are side-by-side
  height = 5,
  dpi = 600
)
```
...
forest_plot <- ggforest(cox_fit, 
         ...
         noDigits = 2) 
ggsave(
  filename = "Figure_3_Cox_Forest_Plot.png",
  plot = forest_plot,
  width = 10,
  height = 6,
  dpi = 600,
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=15, fig.height=6, fig.align='center', out.width="100%"}
...
cuminc_combined <- (plot1 | plot2) +
  plot_annotation(title = "Kaplan-Meier Curves with Compact Risk Tables")

ggsave(
  filename = "Figure_5_Cumulative_Incidence_Curves.png",
  plot = cuminc_combined,
  width = 14,
  height = 8,
  dpi = 600,
  bg = "white"
...
# Plot
varimp_plot <- ggplot(tidy_cox, aes(x = reorder(term, abs(estimate)), y = estimate)) +
...
ggsave(
  filename = "Figure_6_Cox_Variable_Importance.png",
  plot = varimp_plot,
  width = 8,
  height = 5,
  dpi = 600,
...
combined_exploratory <- viz_3|viz_4

ggsave(
  filename = "Figure_7_Exploratory_Data_Analysis.png",
  plot = combined_exploratory,
  width = 14,
  height = 6,
  dpi = 600,
  bg = "white"
